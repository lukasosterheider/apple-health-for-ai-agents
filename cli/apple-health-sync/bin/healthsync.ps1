...
if (Test-Path -LiteralPath $BundledRuntime -PathType Leaf) {
    ...
    if (-not (Test-Path -LiteralPath $VersionFile)) { $VersionFile = Join-Path $PluginRoot "VERSION.txt" }
    if (Test-Path -LiteralPath $VersionFile -PathType Leaf) {
        $ExpectedVersion = (Get-Content -LiteralPath $VersionFile -Raw).Trim()
        $env:HEALTHSYNC_EXPECTED_VERSION = $ExpectedVersion
    }
    switch ($RuntimeAction) {
        ...
        "verify" {
            ...
            if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
            Write-Output "Apple Health Sync bundled runtime self-test passed (network not tested). Run healthsync network-diagnostics to test HTTPS."
            exit 0
        }
        "clean" {
...
if (-not (Test-Path -LiteralPath $ManifestPath -PathType Leaf)) {
    Fail "No bundled executable and no release manifest: $ManifestPath. Install a complete package from https://github.com/lukasosterheider/apple-health-for-ai-agents/releases"
}
...
$VersionFile = Join-Path $PluginRoot "VERSION"
if (-not (Test-Path -LiteralPath $VersionFile)) { $VersionFile = Join-Path $PluginRoot "VERSION.txt" }
if ((Test-Path -LiteralPath $VersionFile) -and (Get-Content -LiteralPath $VersionFile -Raw).Trim() -ne $RuntimeVersion) {
    Fail "Runtime manifest version does not match this package."
}
$env:HEALTHSYNC_EXPECTED_VERSION = $RuntimeVersion
...
switch ($RuntimeAction) {
    ...
    "verify" {
        ...
        Write-Output "Apple Health Sync runtime $RuntimeVersion self-test passed for $PlatformTag (network not tested). Run healthsync network-diagnostics to test HTTPS."
        exit 0
    }
    "clean" {
