...
metadata: {"openclaw":{"homepage":"https://gethealthsync.app/","requires":{"bins":["sh"]},"config":{"stateDirs":[".apple-health-sync"]}}}
---
...
- Run `sh {baseDir}/bin/healthsync.sh <command>` on macOS/Linux or `powershell -File bin/healthsync.ps1 <command>` on Windows. The examples below abbreviate that exact launcher as `healthsync`; do not substitute an unrelated executable on PATH.
